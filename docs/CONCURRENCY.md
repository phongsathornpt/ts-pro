...
15. `runtime: add Green-Tea-style local mark-page work` 🟡
    ...
    - [x] Stack-allocate proven non-escaping closures only in acyclic, non-suspending functions. The closure cell and capture environment become parent-frame `alloca`s; reference captures stay protected by their original SSA GC roots, while returned/call-escaped/task-crossing closures remain heap-backed. Native GC-churn acceptance covers a captured string surviving until the indirect call.
