...
7. **Crypto/performance/compression**: `performance`, `crypto.getRandomValues(Uint8Array)`, `crypto.randomUUID()`, and the first `SubtleCrypto` algorithm families are implemented. Random APIs reuse the OS-backed secure random source; `getRandomValues` preserves `Uint8Array` view offsets and enforces the 65,536-byte Web Crypto quota, while `randomUUID` emits canonical lowercase RFC 4122 version-4 UUIDs with the required version and variant bits. `SubtleCrypto.digest()` snapshots `ArrayBuffer` / `Uint8Array` input bytes at call time, accepts either string algorithm identifiers or `{ name }` algorithm dictionaries, normalizes SHA-1/SHA-256/SHA-384/SHA-512 names ASCII-case-insensitively, uses dependency-free raw-native digest primitives, returns a Promise-like `ArrayBuffer`, and rejects unsupported names with `NotSupportedError`. HMAC supports key generation/import/export plus sign/verify, and HKDF supports raw-key import plus SHA-256 `deriveBits()`. AES-GCM now has a deliberately bounded Linux/AMD64 foundation backed by the existing native AEAD primitive: raw 128-bit key import, `encrypt()` / `decrypt()`, a 96-bit IV, explicit additional data, and a 128-bit authentication tag, with `CryptoKey` metadata/usages and rejection semantics covered by native fixtures. The remaining ECMA-429 WebCrypto operations/key algorithms and compression/decompression streams remain.
