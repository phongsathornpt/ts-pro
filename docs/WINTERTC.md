...
7. **Crypto/performance/compression**: `performance` and the initial `crypto.getRandomValues(Uint8Array)` surface are implemented with OS-backed secure randomness, `Uint8Array` view-offset preservation, the 65,536-byte Web Crypto quota, and Web-compatible `DOMException` errors. `CryptoKey`, `SubtleCrypto`, the remaining `Crypto` surface, and compression/decompression streams remain.
