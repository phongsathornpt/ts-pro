...
7. **Crypto/performance/compression**: `performance`, `crypto.getRandomValues(Uint8Array)`, `crypto.randomUUID()`, and the initial `crypto.subtle.digest()` surface are implemented. Random APIs reuse the OS-backed secure random source; `getRandomValues` preserves `Uint8Array` view offsets and enforces the 65,536-byte Web Crypto quota, while `randomUUID` emits canonical lowercase RFC 4122 version-4 UUIDs with the required version and variant bits. `SubtleCrypto.digest()` snapshots `ArrayBuffer` / `Uint8Array` input bytes at call time, accepts either string algorithm identifiers or `{ name }` algorithm dictionaries, normalizes SHA-256 names ASCII-case-insensitively, uses the existing native SHA-256 primitive, returns a Promise-like `ArrayBuffer`, and rejects unsupported names with `NotSupportedError`. The remaining required digest algorithms, `CryptoKey`, the rest of `SubtleCrypto`, and compression/decompression streams remain.
