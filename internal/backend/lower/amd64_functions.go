...
func lowerAMD64Functions(e *amd64.Emitter, prog *ir.Program, fnOffsets map[string]int, callFixupsOut *[]callFixup, branchFixupsOut *[]branchFixupAMD64, strFixupsOut *[]stringFixupAMD64, closureCodeFixupsOut *[]closureCodeFixupAMD64, bbOffsets map[*ir.BasicBlock]int) error {
	callFixups := *callFixupsOut
	branchFixups := *branchFixupsOut
	strFixups := *strFixupsOut
	closureCodeFixups := *closureCodeFixupsOut
			...
			for _, inst := range bb.Instructions {
				...
				case *ir.BinaryInst:
					...
					if isNumberType(bi.LHS.Type()) || isNumberType(bi.RHS.Type()) {
						...
						default:
							return fmt.Errorf("unsupported AMD64 number op %v", bi.Op)
						}
					...
					case ir.OpNe:
						...
						e.Setcc(amd64.CondNE, amd64.R10)
					default:
						return fmt.Errorf("unsupported AMD64 non-number op %v", bi.Op)
					}
					storeValue(dstLoc, amd64.R10)

				case *ir.UnaryInst:
					if bi.Op != "-" || !isNumberType(bi.Val.Type()) {
						return fmt.Errorf("unsupported AMD64 unary op %q", bi.Op)
					}
	...
	*callFixupsOut = callFixups
	*branchFixupsOut = branchFixups
	*strFixupsOut = strFixups
	*closureCodeFixupsOut = closureCodeFixups
	return nil
}
