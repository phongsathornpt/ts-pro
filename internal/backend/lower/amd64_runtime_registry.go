...
func emitAMD64RuntimeSymbols(e *amd64.Emitter, fnOffsets map[string]int) {
	...
	fnOffsets["ts_task_mark_rejection_reported"] = len(e.Code)
	emitAMD64TaskMarkRejectionReported(e)
	fnOffsets["ts_task_rejection_reported"] = len(e.Code)
	emitAMD64TaskRejectionReported(e)
	...
	fnOffsets["ts_crypto_sha256_compress"] = len(e.Code)
	emitAMD64SHA256Compress(e)
	fnOffsets["ts_crypto_sha256"] = len(e.Code)
	emitAMD64SHA256(e, fnOffsets["ts_crypto_sha256_compress"], fnOffsets["ts_byte_buffer_new"])
	fnOffsets["ts_crypto_hmac_sha256"] = len(e.Code)
	emitAMD64HMACSHA256(e, fnOffsets["ts_crypto_sha256"], fnOffsets["ts_byte_buffer_new"])
	fnOffsets["ts_crypto_hkdf_extract_sha256"] = len(e.Code)
	emitAMD64HKDFExtract(e, fnOffsets["ts_crypto_hmac_sha256"])
	fnOffsets["ts_crypto_hkdf_expand_sha256"] = len(e.Code)
	emitAMD64HKDFExpand(e, fnOffsets["ts_crypto_hmac_sha256"], fnOffsets["ts_byte_buffer_new"], fnOffsets["ts_byte_buffer_copy"])
	fnOffsets["ts_crypto_aes_128_gcm_encrypt"] = len(e.Code)
	emitAMD64AES128GCMEncrypt(e, fnOffsets["ts_byte_buffer_new"], fnOffsets["ts_byte_buffer_copy"])
	fnOffsets["ts_crypto_aes_128_gcm_decrypt"] = len(e.Code)
	emitAMD64AES128GCMDecrypt(e, fnOffsets["ts_byte_buffer_new"], fnOffsets["ts_byte_buffer_copy"])
	fnOffsets["ts_tls13_hkdf_expand_label"] = len(e.Code)
	emitAMD64TLS13HKDFExpandLabel(e, fnOffsets["ts_crypto_hkdf_expand_sha256"], fnOffsets["ts_byte_buffer_new"], fnOffsets["ts_byte_buffer_copy"])
	fnOffsets["ts_tls13_nonce"] = len(e.Code)
	emitAMD64TLS13Nonce(e, fnOffsets["ts_byte_buffer_new"], fnOffsets["ts_byte_buffer_copy"])
	fnOffsets["ts_tls13_encrypt_record"] = len(e.Code)
	emitAMD64TLS13EncryptRecord(e, fnOffsets["ts_tls13_nonce"], fnOffsets["ts_crypto_aes_128_gcm_encrypt"], fnOffsets["ts_byte_buffer_new"], fnOffsets["ts_byte_buffer_copy"])
	fnOffsets["ts_tls13_decrypt_record"] = len(e.Code)
	emitAMD64TLS13DecryptRecord(e, fnOffsets["ts_tls13_nonce"], fnOffsets["ts_crypto_aes_128_gcm_decrypt"], fnOffsets["ts_byte_buffer_new"], fnOffsets["ts_byte_buffer_copy"])
	fnOffsets["ts_os_random"] = len(e.Code)
	emitAMD64OSRandom(e, fnOffsets["ts_byte_buffer_new"])
	fnOffsets["ts_crypto_fe_add"] = len(e.Code)
	emitAMD64FEAdd(e)
	fnOffsets["ts_crypto_fe_sub"] = len(e.Code)
	emitAMD64FESub(e)
	fnOffsets["ts_crypto_fe_mul"] = len(e.Code)
	emitAMD64FEMul(e)
	fnOffsets["ts_crypto_fe_square"] = len(e.Code)
	emitAMD64FESquare(e, fnOffsets["ts_crypto_fe_mul"])
	fnOffsets["ts_crypto_fe_mul121665"] = len(e.Code)
	emitAMD64FEMul121665(e)
	fnOffsets["ts_crypto_x25519"] = len(e.Code)
	emitAMD64X25519(e, fnOffsets["ts_crypto_fe_add"], fnOffsets["ts_crypto_fe_sub"], fnOffsets["ts_crypto_fe_mul"], fnOffsets["ts_crypto_fe_square"], fnOffsets["ts_crypto_fe_mul121665"], fnOffsets["ts_byte_buffer_new"])
	...
	fnOffsets["ts_net_http_open_ipv4"] = len(e.Code)
	emitAMD64NetHTTPOpenIPv4(e, fnOffsets["ts_task_yield"])
	fnOffsets["ts_net_http_open_ipv6"] = len(e.Code)
	emitAMD64NetHTTPOpenIPv6(e, fnOffsets["ts_task_yield"])
	fnOffsets["ts_net_http_write"] = len(e.Code)
	emitAMD64NetHTTPWrite(e, fnOffsets["ts_task_yield"])
	...
	emitAMD64ArrayGet(e)
	fnOffsets["ts_js_index_get"] = len(e.Code)
	emitAMD64JSIndexGet(e, fnOffsets["ts_array_get"])
	...
	fnOffsets["ts_utf8_from_code_point"] = len(e.Code)
	emitAMD64UTF8FromCodePoint(e, fnOffsets["ts_alloc"])
	fnOffsets["ts_json_number_valid"] = len(e.Code)
	emitAMD64JSONNumberValid(e)
