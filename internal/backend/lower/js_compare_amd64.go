...
func emitAMD64JSStringCompare(e *amd64.Emitter) {
	...
	// JavaScript compares strings lexicographically by UTF-16 code units, not
	// by the UTF-8 bytes used by ts-pro's internal string representation.
	e.Push(amd64.RBP)
	e.MovRegReg(amd64.RBP, amd64.RSP)
	e.Push(amd64.RBX)
	e.Push(amd64.R12)
	...
	e.MovRegReg(amd64.RBX, amd64.RDI)
	e.AddRegImm32(amd64.RBX, 8)
	e.MovRegDeref(amd64.R12, amd64.RDI, 0)
	e.MovRegReg(amd64.R13, amd64.RSI)
	e.AddRegImm32(amd64.R13, 8)
	e.MovRegDeref(amd64.R14, amd64.RSI, 0)
	e.MovRegImm64(amd64.R10, 0)
	e.MovDerefReg(amd64.RSP, 0, amd64.R10)
	e.MovDerefReg(amd64.RSP, 8, amd64.R10)

	emitNextUnit := func(ptr, remaining amd64.Register, pending int32) {
		// Return the next UTF-16 code unit in RAX. 0x110000 is an end sentinel.
		e.MovRegDeref(amd64.RAX, amd64.RSP, pending)
		e.TestRegReg(amd64.RAX, amd64.RAX)
		noPending := len(e.Code)
		e.JccRel32(amd64.CondE, 0)
		e.MovRegImm64(amd64.R10, 0)
		e.MovDerefReg(amd64.RSP, pending, amd64.R10)
		pendingDone := len(e.Code)
		e.JmpRel32(0)
		patchJcc(noPending, len(e.Code))

		e.TestRegReg(remaining, remaining)
		hasBytes := len(e.Code)
		e.JccRel32(amd64.CondNE, 0)
		e.MovRegImm64(amd64.RAX, 0x110000)
		endDone := len(e.Code)
		e.JmpRel32(0)
		patchJcc(hasBytes, len(e.Code))

		e.MovzxRegDeref8(amd64.RAX, ptr, 0)
		e.CmpRegImm32(amd64.RAX, 0x80)
		nonASCII := len(e.Code)
		e.JccRel32(amd64.CondAE, 0)
		e.AddRegImm32(ptr, 1)
		e.SubRegImm32(remaining, 1)
		asciiDone := len(e.Code)
		e.JmpRel32(0)
		patchJcc(nonASCII, len(e.Code))

		e.CmpRegImm32(amd64.RAX, 0xE0)
		threeOrFour := len(e.Code)
		e.JccRel32(amd64.CondAE, 0)
		// 2-byte sequence.
		...
		e.MovzxRegDeref8(amd64.R11, ptr, 1)
		e.MovRegImm64(amd64.R10, 0x3F)
		e.AndRegReg(amd64.R11, amd64.R10)
		e.AddRegReg(amd64.RAX, amd64.R11)
		e.AddRegImm32(ptr, 2)
		e.SubRegImm32(remaining, 2)
		twoDone := len(e.Code)
		e.JmpRel32(0)

		patchJcc(threeOrFour, len(e.Code))
		e.CmpRegImm32(amd64.RAX, 0xF0)
		fourByte := len(e.Code)
		e.JccRel32(amd64.CondAE, 0)
		// 3-byte sequence.
		e.MovRegImm64(amd64.R10, 0x0F)
		e.AndRegReg(amd64.RAX, amd64.R10)
		e.MovRegImm64(amd64.R10, 4096)
		e.ImulRegReg(amd64.RAX, amd64.R10)
		e.MovzxRegDeref8(amd64.R11, ptr, 1)
		e.MovRegImm64(amd64.R10, 0x3F)
		e.AndRegReg(amd64.R11, amd64.R10)
		...
		e.AddRegReg(amd64.RAX, amd64.R11)
		e.MovzxRegDeref8(amd64.R11, ptr, 2)
		e.MovRegImm64(amd64.R10, 0x3F)
		e.AndRegReg(amd64.R11, amd64.R10)
		e.AddRegReg(amd64.RAX, amd64.R11)
		e.AddRegImm32(ptr, 3)
		e.SubRegImm32(remaining, 3)
		threeDone := len(e.Code)
		e.JmpRel32(0)

		patchJcc(fourByte, len(e.Code))
		// 4-byte sequence -> surrogate pair. Return high now, retain low.
		e.MovRegImm64(amd64.R10, 0x07)
		e.AndRegReg(amd64.RAX, amd64.R10)
		e.MovRegImm64(amd64.R10, 262144)
		e.ImulRegReg(amd64.RAX, amd64.R10)
		e.MovzxRegDeref8(amd64.R11, ptr, 1)
		e.MovRegImm64(amd64.R10, 0x3F)
		e.AndRegReg(amd64.R11, amd64.R10)
		...
		e.ImulRegReg(amd64.R11, amd64.R10)
		e.AddRegReg(amd64.RAX, amd64.R11)
		e.MovzxRegDeref8(amd64.R11, ptr, 3)
		e.MovRegImm64(amd64.R10, 0x3F)
		e.AndRegReg(amd64.R11, amd64.R10)
		e.AddRegReg(amd64.RAX, amd64.R11)
		e.SubRegImm32(amd64.RAX, 0x10000)
		e.MovRegReg(amd64.R11, amd64.RAX)
		e.MovRegImm64(amd64.R10, 0x3FF)
		e.AndRegReg(amd64.R11, amd64.R10)
		e.AddRegImm32(amd64.R11, 0xDC00)
		e.MovDerefReg(amd64.RSP, pending, amd64.R11)
		e.ShrRegImm8(amd64.RAX, 10)
		e.AddRegImm32(amd64.RAX, 0xD800)
		e.AddRegImm32(ptr, 4)
		e.SubRegImm32(remaining, 4)

		done := len(e.Code)
		for _, at := range []int{pendingDone, endDone, asciiDone, twoDone, threeDone} {
			patchJmp(at, done)
		}
	}

	loop := len(e.Code)
	emitNextUnit(amd64.RBX, amd64.R12, 0)
	e.MovDerefReg(amd64.RSP, 16, amd64.RAX)
	emitNextUnit(amd64.R13, amd64.R14, 8)
	e.MovRegDeref(amd64.RDX, amd64.RSP, 16)
	e.CmpRegReg(amd64.RDX, amd64.RAX)
	less := len(e.Code)
	e.JccRel32(amd64.CondB, 0)
	greater := len(e.Code)
	e.JccRel32(amd64.CondA, 0)
	e.CmpRegImm32(amd64.RAX, 0x110000)
	equal := len(e.Code)
	e.JccRel32(amd64.CondE, 0)
	back := len(e.Code)
	e.JmpRel32(0)
	...
	e.MovRegImm64(amd64.RAX, -1)
	resultDone1 := len(e.Code)
	e.JmpRel32(0)
	greaterLabel := len(e.Code)
	patchJcc(greater, greaterLabel)
	e.MovRegImm64(amd64.RAX, 1)
	resultDone2 := len(e.Code)
	e.JmpRel32(0)
	equalLabel := len(e.Code)
	patchJcc(equal, equalLabel)
	e.MovRegImm64(amd64.RAX, 0)
	done := len(e.Code)
	patchJmp(resultDone1, done)
	patchJmp(resultDone2, done)
