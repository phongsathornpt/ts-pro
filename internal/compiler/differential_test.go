...
func TestPureGoOutputMatchesTypeScriptReference(t *testing.T) {
	...
	fixtures := []string{
		"fib.ts", "scalars.ts", "loops.ts", "do_while.ts", "switch_control_flow.ts", "nullish_coalescing.ts", "template_literals.ts", "tuples_and_destructuring.ts", "enums_and_defaults.ts", "for_of_loops.ts", "json_api.ts", "rest_and_spread.ts", "map_set.ts", "date_api.ts", "regexp_api.ts", "computed_property_keys.ts", "optional_properties.ts", "evolving_shapes.ts", "generics.ts", "arrays.ts", "array_writes.ts", "array_push_pop.ts", "string_arrays.ts",
		...
		"multi_module.ts", "promise_completeness.ts",
	}
...
func compareReferenceOutput(t *testing.T, root, tsc, node, fixture string, pureGo bool) {
	...
	args := []string{"--ignoreConfig", input, filepath.Join(root, "types", "tsnative-runtime.d.ts"),
		"--target", "ES2022", "--module", "CommonJS", "--moduleResolution", "Bundler",
	...
	var js string
	expectedBase := strings.TrimSuffix(fixture, ".ts") + ".js"
	_ = filepath.Walk(refDir, func(path string, info os.FileInfo, err error) error {
		if err == nil && !info.IsDir() && filepath.Base(path) == expectedBase {
			js = path
		}
		return nil
	})
